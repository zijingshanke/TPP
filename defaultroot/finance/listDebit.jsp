...
function download(){
	...
	document.forms[0].action="../transaction/debitlist.do?thisAction=downloadDebit";
	document.forms[0].submit();
	document.forms[0].action="../transaction/debitlist.do?thisAction=listDebit";
}
function revocation(id){
	window.location.href="../transaction/debit.do?thisAction=revocationDebit&id="+id;
}

function edit(id)
{
		window.location.href="../transaction/debit.do?thisAction=editDebit&id="+id;
}
</script>
	...
	<body>
		<div id="mainContainer">
			<div id="container">
				<html:form action="/transaction/debitlist.do?thisAction=listDebit" >
...
<c:import url="../_jsp/mainTitle.jsp?title1=交易管理&title2=预支列表" charEncoding="UTF-8" />
								...
								<table width="100%" cellpadding="0" cellspacing="0" border="0"
									...
									<c:forEach var="info" items="${dlf.list}" varStatus="status">
										<tr>
											...
											<td>
											<html:link
													page="/transaction/debitLoglist.do?thisAction=listDebitByContent&orderNo=${info.no}">
												<c:out value="${info.no}" />
												</html:link>
											</td>
											<td>
											...
											<td><FONT color="red;"><c:out value="${info.expenseStatus}" /></td>
											<td>
								...
								<table width="100%" style="margin-top: 5px;">
									<tr> 
										<td><input type="button" value="下载预支报表" class="button2" onclick="download();"/></td>
